...
export default function Home() {
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully minted Block Dev Tokens");
  ...
  const renderButton = () => {
    ...
    if (loading) {
      return (
        <button className="mx-auto bg-blue-600 flex-row flex rounded-md shadow-md hover:opacity-0.8 cursor-pointer text-white items-center my-2">
    ...
    if (tokensToBeClaimed > 0) {
      return (
        <div>
          <TextBox text={`${tokensToBeClaimed * 10} Tokens can be claimed!`} />
          <button
            onClick={claimCryptoDevTokens}
            className="mx-auto bg-blue-600 px-5 py-4 rounded-md shadow-md hover:opacity-0.8 cursor-pointer text-white"
          >
            Claim your Block Dev Coins
          </button>
        </div>
      );
    }
    return (
      <div className="flex flex-col">
        <input
          className="border-black border-solid p-2 border rounded-lg my-5"
          type="number"
        ...
        <div className="flex justify-center">
          <ul className="flex flex-col py-10 content-between text-left justify-center">
            <li className="font-bold text-center text-xl md:text-3xl md:my-10">
              Welcome to Block Devs ICO
            </li>
            <li className="px-3 my-3 text-lg md:text-xl md:my-0">
              You can claim or mint Block Dev tokens here
            </li>
            ...
            <li className="px-3 flex justify-center md:block flex-col">
              <div>
                {walletConnected ? (
                  <div>
                    <ul>
                      <li className="px-3 my-3 text-lg md:text-xl">
                        You have minted{" "}
                        {utils.formatEther(balanceOfCryptoDevTokens)} Block Dev
                        Tokens
                      </li>
                      <li className="px-3 my-3 text-lg md:text-xl">
                        Overall {utils.formatEther(tokensMinted)}/10000 have
                        been minted.
                      </li>
                    </ul>
                  ...
                  <button
                    ...
                    className="mx-auto bg-blue-600 px-5 py-4 rounded-md shadow-md hover:opacity-0.8 cursor-pointer text-white my-5"
                  >
                    Connect your Wallet
                  </button>
                )}
